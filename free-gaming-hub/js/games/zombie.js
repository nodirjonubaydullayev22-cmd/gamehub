/* ==========================================================================
   Zombie Survival Engine - Free Gaming Hub
   Features: Top-down shooter, Zombie wave spawner, Weapon ammo & upgrades
   ========================================================================== */
...
class ZombieGame {
  constructor(canvas, scoreEl, bestScoreEl, onGameOver) {
    ...
    this.bestScore = StorageManager.getBestScore('zombie');
      ...
      this.bullets.forEach((b, bIdx) => {
        if (Math.hypot(b.x - z.x, b.y - z.y) < z.r + 5) {
          ...
          if (z.hp <= 0) {
            ...
            if (this.zombiesKilled >= 20) {
              StorageManager.unlockAchievement('zombie_hunter');
            }
          }
        }
      });
  ...
  updateHUD() {
    if (this.scoreEl) this.scoreEl.textContent = `Score: ${this.score} | Wave: ${this.wave} | HP: ${Math.max(0, Math.floor(this.player.hp))}`;
    if (this.bestScoreEl) this.bestScoreEl.textContent = this.bestScore;
  }

  gameOver() {
    this.isRunning = false;
    const saveRes = StorageManager.saveScore('zombie', 'Zombie Survival', this.score);
